...
def main(task='text_generation',
    ...
    if run_mode == 'predict':
        ...
        batch_input = []
        with open(predict_data, 'r', encoding='utf-8') as file:
            content = file.read()
            matches = re.findall(r'prompt\d*: (.+?)(?=prompt\d*|$)', content, re.DOTALL)
            batch_input = [match.strip() for match in matches]
        with open('qwen1_5_72b_text_generation_result.txt', 'w') as f:
            prompt_count = 0
            for input_prompt in batch_input:
                ...
                prompt_count += 1
                prompt_text = f"prompt {prompt_count}:\n"
                f.write(prompt_text + result[0]["text_generation_text"][0] + '\n\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--task', default='text_generation', type=str,
                        help='set task type.')
    parser.add_argument('--config', default='run_qwen1_5_72b_infer.yaml', type=str,
